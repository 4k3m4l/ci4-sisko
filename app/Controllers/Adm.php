...
use CodeIgniter\Models\Adm_model;
...
/*====================================================================================================== */
	...
	public function buku_induk()
	{
		...
		if ($ke == "cari") {
			...
			echo view('adm/v_buku_induk', $adm);
		} else if ($ke == "add") {
			...
			echo view('adm/f_buku_induk', $adm);
		} else if ($ke == "act_add") {
			//validasi form
			...
			$id	= $this->uri->segment(4);
			$adm['data_siswa']	= $this->adm_model->delData("ts_data_siswa", "id", $id);
			$adm['gemar']		= $this->adm_model->delData("ts_gemar", "id_siswa", $id);
			$adm['kembang_siswa'] = $this->adm_model->delData("ts_kembang_siswa", "id_siswa", $id);
			$adm['kesehatan']	= $this->adm_model->delData("ts_kesehatan", "id_siswa", $id);
			$adm['ortu_ayah']	= $this->adm_model->delData("ts_ortu_ayah", "id_siswa", $id);
			$adm['ortu_ibu']	= $this->adm_model->delData("ts_ortu_ibu", "id_siswa", $id);
			$adm['ortu_wali']	= $this->adm_model->delData("ts_ortu_wali", "id_siswa", $id);
			$adm['pend_sebelum'] = $this->adm_model->delData("ts_pend_sebelum", "id_siswa", $id);
			$adm['setelah']		= $this->adm_model->delData("ts_setelah", "id_siswa", $id);
			$adm['tmp_tinggal']	= $this->adm_model->delData("ts_tmp_tinggal", "id_siswa", $id);
	...
	public function siswa_kelas()
	{
		...
		echo view('adm/t_header');
		echo view('adm/sidebar');
		echo view('adm/v_siswa_per_kelas', $adm);
	}
	//End Siswa Kelas
	...
	public function guru_duk()
	{
		...
		if ($ke == "pilih_pegawai") {
			...
			echo view('adm/f_guru_duk_pilih_pegawai.php', $adm);
		} else if ($ke == "add") {
			...
			if ($cek_pgw_jml == 0) {
				...
				echo view('adm/f_guru_duk.php', $adm);
			} else {
			...
			echo view('adm/v_guru_duk.php', $adm);
		}
	}
	//End Guru DUK
	...
	public function guru_kepeg()
	{
		...
		if ($ke == "add") {
			...
			echo view('adm/f_guru_kepeg', $adm);
		} else if ($ke == "edt") {
	...
	public function g_bangunan()
	{
		...
		if ($ke == "cari") {
			...
			echo view('adm/v_g_bangunan', $adm);
		} else if ($ke == "hps") {
			...
			echo view('adm/f_g_bangunan', $adm);
		} else if ($ke == "act_add") {
	...
	public function no_tanah_bgn()
	{
		...
		if ($ke == "cari") {
			...
			echo view('adm/v_no_tanah_bgn', $adm);
		} else if ($ke == "hps") {
			...
			$adm['ruang']		= $this->adm_model->getAll('ti_ruang');
			$adm['inv'] 		= $this->adm_model->getAll('ti_invent ORDER BY kd_brg, no_aset');
			echo view('adm/f_no_tanah_bgn', $adm);
		} else if ($ke == "act_add") {
	...
	public function dir()
	{
		...
		if ($ke == "cari") {
			...
			$adm['ruang'] = $this->adm_model->getAll('ti_ruang');
			echo view('adm/f_pindah_dir', $adm);
	...
	public function pindah_kelas()
	{
		...
		FROM (ts_data_siswa LEFT JOIN tl_siswa_kelas ON ts_data_siswa.id=tl_siswa_kelas.id_siswa) LEFT JOIN tl_kelas ON tl_siswa_kelas.kelas=tl_kelas.id WHERE (tl_siswa_kelas.kelas != '99' OR ts_data_siswa.stat_aktif = 'Aktif') AND ts_data_siswa.nama LIKE '%" . $this->input->post('q') . "%' ")->result();
			echo view('adm/v_siswa_kelas', $adm);
	...
	public function input_nilai_per_kelas()
	{
		...
		if ($ke == "f_input_per_kelas") {
			...
			if (trim($id_kelas) == "" || trim($id_mapel) == "" || trim($sem) == "") {
				...
				echo view('adm/v_input_per_kelas', $adm);
			} else {
				...
				if ($cek_sudah == 0) {
					...
					echo view('adm/f_input_per_kelas', $adm);
				} else {
	...
	public function jadwal()
	{
		...
		if ($ke != "excel") {
			...
			if ($ke == "add") {
				$adm['kelas']	= $this->db->query("SELECT id, nama FROM tl_kelas WHERE id != 99")->result();
				echo view('adm/f_jadwal_pilih_hari.php', $adm);
				...
				if ($q_cek_ada_jadwal == 0) {
					...
					echo view('adm/f_jadwal.php', $adm);
				} else {
				...
				echo view('adm/v_jadwal.php', $adm);
			}
		} else {
			echo view("excel/jadwal", $adm);
		}
	}
	...
	public function jadwal_by_guru()
	{
		...
		if ($ke == "det") {
			...
			echo view('adm/v_jadwal_by_guru', $adm);
		} else {
			echo view('adm/v_jadwal_by_guru', $adm);
		}
	}
	...
	public function jadwal_by_kelas()
	{
		...
		if ($ke == "det") {
			...
			echo view('adm/v_jadwal_by_kelas', $adm);
		} else {
			echo view('adm/v_jadwal_by_kelas', $adm);
		}
	}
	...
	public function jadwal_by_hari()
	{
		...
		if ($ke == "det") {
			...
			echo view('adm/v_jadwal_by_hari', $adm);
		} else {
			echo view('adm/v_jadwal_by_hari', $adm);
		}
	}
